...
export function ProjectsView() {
  ...
  function openCreate() {
    ...
    setForm({
      ...emptyProject(),
      managerId: snapshot.technicians[0]?.id ?? "",
    });
    setFormError("");
    setOpen(true);
  }
    ...
    <>
      ...
      <div className="mb-5 flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
        <div className="flex flex-1 flex-wrap gap-2 xl:flex-nowrap">
          <div className="relative min-w-128 flex-1 lg:max-w-sm">
            ...
            {query && (
              <button
                type="button"
                onClick={() => setQuery("")}
                className="absolute right-2 top-1/2 -translate-y-1/2 cursor-pointer rounded p-0.5 text-slate-400 hover:text-slate-700"
              >
        ...
        <div className="flex gap-2 items-center">
          ...
          <Button className="flex" onClick={openCreate}>
      ...
      {view === "timeline" ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((project) => {
            const manager = snapshot.technicians.find(
              (item) => item.id === project.managerId,
            );
            return (
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between">
                    <Link href={`/projects/${project.id}`} className="min-w-0">
                      <h3 className="truncate font-semibold text-slate-950 hover:text-orange-600">
                        {project.name}
                      </h3>
      ...
      {!filtered.length && (
        <Card className="border-dashed p-16 text-center text-sm text-slate-500">
