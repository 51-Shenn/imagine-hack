...
import { IconCalendarClock, IconDiamond, IconPencil } from "@tabler/icons-react";
...
export function ProjectDetailTimeline({ project, tasks, initialSubtasks, members }: { project: Project; tasks: Task[]; initialSubtasks: Subtask[]; members: TeamMember[] }) {
  ...
  if (deadlines.length === 0) return <Card className="flex min-h-72 flex-col items-center justify-center border-dashed p-8 text-center"><IconCalendarClock className="size-8 text-slate-300" /><p className="mt-3 font-medium text-slate-800">No deadlines available</p><p className="mt-1 text-sm text-slate-500">Add deadlines to tasks and subtasks to build this timeline.</p></Card>;
      ...
      <div className="grid" style={{ gridTemplateColumns: `320px ${chartWidth}px`, minWidth: 320 + chartWidth }}>
        ...
        {taskItems.flatMap(task => {
          ...
          childSubtasks.forEach(subtask => {
            ...
            rows.push(<div key={`${subtask.id}-label`} className="sticky left-0 z-20 flex min-h-14 items-center border-b border-r border-slate-200 bg-slate-50/60 py-2 pl-9 pr-4"><div className="min-w-0 flex-1"><p className="truncate text-xs font-medium text-slate-700">{subtask.title}</p><label className="mt-1 flex items-center gap-1.5 text-[10px] text-slate-400"><IconCalendarClock className="size-3" />Deadline<Input type="date" value={inputDate(subtask.dueDate)} onChange={event => updateDeadline(subtask.id, event.target.value)} className="h-6 w-32 px-1.5 text-[10px]" /></label></div></div>);
            rows.push(<div key={`${subtask.id}-chart`} className="relative min-h-14 border-b border-slate-200 bg-slate-50/30" style={gridBackground}>{showToday && <div className="absolute inset-y-0 z-10 w-px bg-orange-400" style={{ left: todayIndex * dayWidth + dayWidth / 2 }} />}<div className="absolute top-1/2 h-1 -translate-y-1/2 rounded-full bg-orange-200" style={{ left: startIndex * dayWidth + dayWidth / 2, width: Math.max(2, (subtaskIndex - startIndex) * dayWidth) }} /><IconDiamond className="absolute top-1/2 size-4 -translate-x-1/2 -translate-y-1/2 fill-orange-500 text-orange-500" style={{ left: subtaskIndex * dayWidth + dayWidth / 2 }} /></div>);
          });
          return rows;
        })}
      </div>
