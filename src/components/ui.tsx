...
export function Button({ className, variant = "default", size = "default", ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "default" | "outline" | "ghost" | "secondary" | "danger"; size?: "default" | "sm" | "icon" }) {
  return <button className={cn("inline-flex items-center justify-center gap-2 rounded-lg font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500/30", variant === "default" && "bg-slate-950 text-white hover:bg-slate-800", variant === "outline" && "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50", variant === "ghost" && "text-slate-600 hover:bg-slate-100 hover:text-slate-950", variant === "secondary" && "bg-orange-50 text-orange-700 hover:bg-orange-100", variant === "danger" && "bg-red-50 text-red-700 hover:bg-red-100", size === "default" && "h-10 px-4 text-sm", size === "sm" && "h-8 px-3 text-xs", size === "icon" && "size-10", className)} {...props} />;
}

export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) { return <div className={cn("rounded-xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)]", className)} {...props} />; }
...
export function CardTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) { return <h3 className={cn("font-semibold tracking-tight text-slate-950", className)} {...props} />; }
...
const badgeStyles: Record<string, string> = {
  in_progress: "bg-emerald-50 text-emerald-700 ring-emerald-600/15", active: "bg-emerald-50 text-emerald-700 ring-emerald-600/15", completed: "bg-emerald-50 text-emerald-700 ring-emerald-600/15", done: "bg-emerald-50 text-emerald-700 ring-emerald-600/15",
  planning: "bg-slate-100 text-slate-600 ring-slate-500/15", todo: "bg-slate-100 text-slate-600 ring-slate-500/15", low: "bg-slate-100 text-slate-600 ring-slate-500/15",
  on_hold: "bg-amber-50 text-amber-700 ring-amber-600/15", review: "bg-violet-50 text-violet-700 ring-violet-600/15", medium: "bg-amber-50 text-amber-700 ring-amber-600/15", on_leave: "bg-amber-50 text-amber-700 ring-amber-600/15",
  high: "bg-red-50 text-red-700 ring-red-600/15", overdue: "bg-red-50 text-red-700 ring-red-600/15",
};
export function Badge({ value, children, className }: { value?: string; children?: React.ReactNode; className?: string }) { const key = value ?? String(children); return <span className={cn("inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold capitalize ring-1 ring-inset", badgeStyles[key] ?? "bg-blue-50 text-blue-700 ring-blue-600/15", className)}>{children ?? key.replaceAll("_", " ")}</span>; }

export function Progress({ value, className, indicatorClassName }: { value: number; className?: string; indicatorClassName?: string }) { return <div className={cn("h-2 overflow-hidden rounded-full bg-slate-100", className)}><div className={cn("h-full rounded-full bg-orange-500 transition-all", indicatorClassName)} style={{ width: `${Math.max(0, Math.min(value, 100))}%` }} /></div>; }

const avatarColors = ["bg-orange-100 text-orange-700", "bg-blue-100 text-blue-700", "bg-violet-100 text-violet-700", "bg-emerald-100 text-emerald-700", "bg-rose-100 text-rose-700"];
export function Avatar({ name, src, size = "md", className }: { name: string; src?: string | null; size?: "sm" | "md" | "lg" | "xl"; className?: string }) { const index = name.charCodeAt(0) % avatarColors.length; return <AvatarPrimitive.Root title={name} className={cn("inline-flex shrink-0 items-center justify-center overflow-hidden rounded-full font-semibold ring-2 ring-white", avatarColors[index], size === "sm" && "size-7 text-[10px]", size === "md" && "size-9 text-xs", size === "lg" && "size-11 text-sm", size === "xl" && "size-16 text-lg", className)}>{src && <AvatarPrimitive.Image src={src} alt={name} className="size-full object-cover" referrerPolicy="no-referrer" />}<AvatarPrimitive.Fallback>{initials(name)}</AvatarPrimitive.Fallback></AvatarPrimitive.Root>; }
...
export function Th({ children, className }: { children?: React.ReactNode; className?: string }) { return <th className={cn("border-b border-slate-200 bg-slate-50/70 px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500", className)}>{children}</th>; }
export function Td({ children, className }: { children: React.ReactNode; className?: string }) { return <td className={cn("border-b border-slate-100 px-4 py-4 text-slate-600", className)}>{children}</td>; }
